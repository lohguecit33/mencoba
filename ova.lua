...
if not _G.Settings then
    _G.Settings = {
        ...
        Other = {
            ["FPS Cap"] = 10, -- Changed from true to 10 to cap FPS at 10
...
coroutine.wrap(pcall)(function()
    if _G.Settings.Other["FPS Cap"] then
        if setfpscap then
            if type(_G.Settings.Other["FPS Cap"]) == "string" or type(_G.Settings.Other["FPS Cap"]) == "number" then
                setfpscap(tonumber(_G.Settings.Other["FPS Cap"]))
                if _G.SendNotifications then
                    StarterGui:SetCore("SendNotification", {
                        Title = "discord.gg/rips",
                        Text = "FPS Capped to " .. tostring(_G.Settings.Other["FPS Cap"]),
                        Duration = 5,
                        Button1 = "Okay"
                    })
                end
                if _G.ConsoleLogs then
                    warn("FPS Capped to " .. tostring(_G.Settings.Other["FPS Cap"]))
                end
